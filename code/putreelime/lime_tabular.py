...
class LimeTabularExplainer(object):
    ...
    def uncertain_sample(data,model):
        # uncertainty sampling (least confident sampling)
        a = model(torch.from_numpy(data).float()).detach().numpy()
        df = pd.DataFrame(np.zeros(shape=(len(a), 2)),columns = ["prob_p","prob_n"])
        df.iloc[:,0]=a
        df.iloc[:,1]=1-a
        df["max_y"] = df.apply(lambda x: max(x["prob_p"],x["prob_n"]),axis=1)
        df["rank"] = df["max_y"].rank()
        return list(df[df["rank"]<=500].index.values)
    ...
    def explain_instance(self,
        ...
        data, inverse = self.__data_inverse(data_row, num_samples, sampling_method,sibling_fn)
        ...
        yss = predict_fn(torch.from_numpy(inverse).float())
    ...
    def __data_inverse(self,
        ...
        inverse[0] = data_row
        if sibling_fn is None:
            return data, inverse
        else:
            uncertain = uncertain_sample(inverse, sibling_fn)
            data = data[uncertain]
            inverse = inverse[uncertain]
            return data, inverse
...
class RecurrentTabularExplainer(LimeTabularExplainer):
    ...
    def explain_instance(self, data_row, classifier_fn, sibling_fn=None,labels=(1,),
